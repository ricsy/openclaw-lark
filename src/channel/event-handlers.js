...
// ---------------------------------------------------------------------------
export async function handleRecallEvent(ctx, data) {
    if (!isEventOwnershipValid(ctx, data))
        return;
    const { accountId, log, error } = ctx;
    try {
        ...
        const operatorOpenId = event.operator_id?.open_id ?? 'unknown';
        log(`feishu[${accountId}]: message ${messageId} recalled by ${operatorOpenId}`);
        // TODO: 触发 Hook 或发送通知
        // 可以通过 runtime 发送消息通知用户，或触发 OpenClaw Hook
    }
    catch (err) {
